...
// ResendCode — повторная отправка кода (для verify-email или reset-password)
func (uc *userUseCase) ResendCode(ctx context.Context, emailAddr, codeType string) error {
	// 1. находим юзера
	...
	// 2. если почта уже подтверждена — повторно слать verify-код бессмысленно
	if codeType == domain.CodeTypeEmailVerify && user.IsVerified {
		return errors.New("email уже подтверждён")
	}

	// 3. проверяем cooldown — смотрим когда был создан последний код
	latest, err := uc.verifyRepo.GetLatestCode(ctx, user.ID, codeType)
	if err != nil {
		return err
	}
	if latest != nil {
	...
	// 5. отправляем письмо — текст зависит от типа кода
...
func (uc *userUseCase) ForgotPassword(ctx context.Context, emailAddr string) error {
	// 1. находим юзера
	user, err := uc.userRepo.GetUserByEmail(ctx, emailAddr)
	if err != nil {
		return err
	...
	latest, err := uc.verifyRepo.GetLatestCode(ctx, user.ID, domain.CodeTypeResetPassword)
	if err != nil {
		return err
	}
	if latest != nil {
	...
	// 4. отправляем код
	return uc.mailer.SendResetPasswordCode(emailAddr, resetCode)
}
