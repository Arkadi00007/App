...
func (r *testRepository) GetTestsBySectionID(ctx context.Context, sectionID int64) ([]*domain.Test, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT t.id, t.section_id, s.subject_id, t.title, t.description,
		       t.show_answer_mode, t.is_premium, t.created_at
		FROM tests t
	...
	for rows.Next() {
		t := &domain.Test{}
		if err := rows.Scan(
			&t.ID, &t.SectionID, &t.SubjectID, &t.Title, &t.Description,
